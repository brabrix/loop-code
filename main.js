...
ipcMain.handle('ai:get', (evt, { projectPath }) =>
  aiCli.resolveProjectAis(loadConfig(), projectPath),
);
...
ipcMain.handle('session:setCli', (evt, { projectPath, sessionId, cli }) => {
  ...
  if (aiCli.VALID_CLIS.includes(cli) && s.cli !== cli) {
    s.cli = cli;
    saveConfig(c);
  }
  return { ok: true };
...
ipcMain.handle('remotes:add', async (evt, { profile, secret }) => {
  ...
  c.remotes[hk] = {
    ...
    remoteDir: (profile.remoteDir || '').trim(),
    label: profile.label || '',
  };
  saveConfig(c);
  ...
  if (!remoteDir) {
    try {
      ...
      remoteDir = await new Promise((resolve, reject) =>
...
ipcMain.handle('remotes:get', (evt, { projectPath }) => {
  ...
  return {
    ...
    port: prof.port,
    user: prof.user,
    authType: prof.authType || 'password',
    keyPath: prof.keyPath || '',
    remoteDir: prof.remoteDir || '/',
    label: prof.label || '',
  };
});
  ...
  (evt, { profile, secret }) =>
    new Promise((resolve) => {
      ...
      const cfg = {
        ...
        hostVerifier: (keyBuf, verify) => {
          ...
          Promise.resolve(confirmHostKey(hk, knownHosts.fingerprint(keyBuf), state))
            .then((ok) => {
              if (ok) knownHosts.trust(hk, keyBuf);
              verify(!!ok);
            })
            .catch(() => verify(false));
        },
      ...
      if (profile.authType === 'key') {
        try {
          cfg.privateKey = fs.readFileSync(profile.keyPath);
        } catch (e) {
          return finish(false, 'Chave: ' + e.message);
        }
...
ipcMain.handle('remote:reconnect', async (evt, { projectPath }) => {
  ...
  try {
    await connections.reconnect(hostKey(projectPath));
    return { ok: true };
  } catch (e) {
...
ipcMain.handle('projects:list', () => {
  ...
  const rail = reconcileRail(
    cfg.rail,
    views.map((v) => v.path),
  );
  if (JSON.stringify(rail) !== JSON.stringify(cfg.rail || [])) {
    cfg.rail = rail;
    saveConfig(cfg);
  }
...
ipcMain.handle('projects:rename', (evt, { projectPath, name }) => {
  const cfg = loadConfig();
  setProjectMeta(cfg, projectPath, {
    name:
      String(name || '')
        .trim()
        .slice(0, 256) || null,
...
ipcMain.handle('fs:dir', async (evt, { dirPath }) => {
  if (isRemote(dirPath)) {
    try {
      return await remoteFs.listDir(dirPath);
    } catch {
      return [];
    }
  }
...
ipcMain.handle('fs:create', (evt, { destDir, name, isDir }) => {
  if (isRemote(destDir)) {
    ...
    const childUri = buildSshUri({
      user: pr.user,
      host: pr.host,
      port: pr.port,
      remoteDir: require('path').posix.join(pr.remoteDir, clean),
    });
  ...
  if (isRemote(destDir) && !isRemote(srcPath)) {
    try {
      ...
      const childUri = buildSshUri({
        ...
        host: pr.host,
        port: pr.port,
        remoteDir: path.posix.join(pr.remoteDir, path.basename(srcPath)),
      });
...
ipcMain.handle('todos:unsubscribe', () => {
  stopTodosWatcher();
  return { ok: true };
});
...
ipcMain.handle('term:ensure', async (evt, { sessionId, projectPath, cols, rows, theme }) => {
  ...
  proc.onData((data) => {
    ...
    if (!remote) {
      captureResumeId(entry);
      activityOnData(entry, data);
    }
...
async function startPhpPreview(projectPath) {
  ...
  console.log(
    `[preview:php] ${path.basename(projectPath)} -> porta ${port} | php ${args.join(' ')}`,
  );
